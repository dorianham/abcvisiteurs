...
function load_animation(type, error="AbcVisiteurs :: Autosurf gratuit")
{
    if(type == "start")
    {
        $("#errors").parent().removeClass("uk-animation-shake");
        displayOverlay('<div uk-spinner="ratio: 0.4"></div> connexion...');
        $(".abcvisiteurs-content div input").attr("disabled", true);
        $(".abcvisiteurs-content div a").attr("disabled", true);
		$(".abcvisiteurs-content div a").html("<span uk-icon='icon: more; ratio: 1.7'></span>");
    }
    if(type == "stop")
    {
        click_able = true;
        ...
        $(".abcvisiteurs-content div input").attr("disabled", false);
        $(".abcvisiteurs-content div a").attr("disabled", false);
		$(".abcvisiteurs-content div a").html("<span uk-icon='icon: bolt; ratio: 1.7'></span>");
    }
    setTimeout(function(){
        click_able = true;
    }, 30000);
    ...
    if(key != "")
    {
        ...
        if(isBase64(data_key))
        {
            ...
            try {
                ...
                res = "Information manquante, essayez un autre identifiant de session.";
            }
            ...
            if(info != null)
            {
                if(info.protocol == "http" || info.protocol == "https")
                {
                    ...
                    authorize_session(call_url, call_data, function(data){
    					if(data["type"] == "success"){
                            ...
                            load_animation("stop", "Identifiant de session invalide, veuillez réessayer.");
    					}
    				});
                } else {
                    res = "Information manquante, essayez un autre identifiant de session.";
                ...
                res = "Identifiant de session invalide, veuillez réessayer.";
            }
        } else {
            res = "Identifiant de session invalide.";
        }
    } else {
        res = "Merci de saisir un identifiant de session.";
    }
    if(res != null)
    {
