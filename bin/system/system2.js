...
const loadstart = () => {
    // show loading
    displayOverlay('<div uk-spinner="ratio: 0.4"></div> Chargement...');
}
    ...
    try{
        ...
        call_browsing(key, function(info){
            ...
            setTimeout( function() {
                ...
                $('.abcvisiteurs-loading a div center svg').css({"opacity": "1"});
			...
			if(first_start == true)
			{
				kill_browsing("Erreur de connexion, merci de réessayer");
			} else {
				$(".abcvisiteurs-address span").html("Reconnexion après 3 secondes...");
				reconnect = setTimeout(function(){
					clearTimeout(reconnect);
					$(".abcvisiteurs-address span").html("Reconnexion...");
					run_browsing(key);
				}, 3000);
			}
		});
...
function update_url(url)
{
    if(ValidURL(url))
    {
        if (url.indexOf("https") != -1) {
            $(".abcvisiteurs-address span").html(url.replace("https", "<span class='ssl'>https</span>"));
            $(".abcvisiteurs-secure .lock").fadeIn(300).find("svg").css({"margin-top":"4px"});
        } else {
            $(".abcvisiteurs-address span").html(url);
            $(".abcvisiteurs-secure .unlock").fadeIn(300);
        }
        return true;
    } else {
        return false;
...
function kill_browsing(text="Accueil")
{
    ...
    $("#report").addClass("disable_area");
    $(".abcvisiteurs-address span").html(text);
    $('.abcvisiteurs-loading a div center svg').css({"opacity": "0.2"});
    ...
    webview.stop();
    webview.setAttribute( 'httpreferrer', "https://www.abcvisiteurs.com/");
    webview.setAttribute( 'useragent', "");
...
function when_its_ready()
{
    ...
    jQuery('#play').click(function(){
        trigger_exchange_btn("wait");
        $('.abcvisiteurs-loading a div center svg').css({"opacity": "0.6"});
